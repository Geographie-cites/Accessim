...
[
  ...
  compteurPasTotal ; pour conna?tre la distance moyenne au service des individus
  max_graphe_temps
  graphe_temps
  max_graphe_temps2
  graphe_temps2
...
[ 
 isMyService?
  ...
  noeudAcces1 ;; noeud par lequel on a acc?s ? l'actualLink de la service
  noeudAcces2 ;; noeud par lequel on a acc?s ? l'actualLink de la service
  sizeInit
  tempsDepuisOuverture
  linkService ;; "adresse" de la boulangerie
  nbPersonneFile ;; Le compteur de personne qui ont attendus, sert ? donner les tickets aux personnes
  tailleFileAttente ;; 
  numeroFileIndividuSuivant ;; le numero appel?
  besoin_insatisfait ;;Quantit? de personne insatisfaite entre deux recalcul de production
  argent
...
[
  ...
  cible             ;; objet cible de l'individu dans son deplacement 
                    ...
                    ;; - soit une service
  cibleReseau ;; lorsque l'individu cherche le r?seau, au d?but
  chercheReseau? ;; true si pas encore sur le r?seau, false si sur le r?seau
  lienReseau ;; lien lorsqu'on se raccroche au r?seau
  noeudReseau ;; noeud lors de l'entree dans le r?seau
  ...
  ;;AJOUT SEBASTIEN
  type_individual ;;Type des individus : explorateur(expl 1), suiveur(suiv 2), errant (suiv 3) indiff?renci?(noname 0)
  ...
  has_statebase ;; 0 si l'on marche, 1 si l'on bloqu? sur un service, 2 si l'on rentre chez nous
  has_muted ;; Est ce que l'individu a chang? d'etat ? Seul moyen de le savoir, soit avec un boolean, soit en interrogeant la m?moire ... on preferera le booleen ici.
  ...
  memEvtChemin ;; La m?moire de l'individu qui contient les evt chemins
]
...
;  
  ...
  ;; Permettrai de mieux controler ce qui se passe, nottament dans les files d'attentes, evt pour le moment qui n'est pas g?r?
...
to CptIndividu_AccrocheReseau
  ...
  [
    ...
    ;La cible est un patch, on r?oriente vers un noeud
    if is-patch? cible and chercheReseau? = false
    [
      if type_individual = 1
      [
        ;;La nouvelle cible est ?gale au noeud le plus proche
...
;  ;; Si cheminFutur = 0, que l'on a pas trouv? le service attendu en tant que suiveur, on met a jour la p?nalit? du chemin dans le quartier, et on devient explorateur
;  ;; FIXME : Ecriture dans une procedure a part si r?utilisation multiple?
;  if (type_individual = 2 AND has_find_service = false AND length(cheminFutur)= 0 )
;  [  
;    ;;On teste si jamais le chemin a ?t? supprim? alors que quelqu'un l'utilise
;    ifelse (chemin idCheminEnCours != nobody)
;    [
;      NewKnow
;      Show word "Suiveur > je n'ai pas trouv? le service indiqu? dans mon chemin > length " length(cheminFutur)
;      
...
;  ]
  ...
  CptIndividu_avancer cible self ;; l'individual avance dans la direction d?finie dans setOrientation
  ...
  ;; Si l'on est sur un noeud ou que l'on rencontre un service, on sauvegarde l'arc entier dans tout les cas (un peu comme si l'on sauvegard? la rue enti?re, et pas que le carrefour)

  ;;La diff?rence d'entregistrement est g?r? dans le savePathMemories
  CptIndividu_savePathMemories self cible has_find_node has_find_service 
      
  ;;On charge en m?moire un chemin si il y'en a un ... autrement dit si on est un suiveur et que l'on passe sur un noeud
  CptIndividu_loadPathMemories has_find_node
      
  ;;Si l'on est sur un service qui est disponible, alors on passe a l'etat d'attente, sinon on continue ? marcher
...
;; Liste la suite de comportement de chaque individu r?alis? a chaque ticks
to askIndividu 
  ...
  [
    ...
    ifelse (has_statebase = 0)[
          ...
          ;;Permet de cr?er d'autre comportements que AccrocheReseau du moment que la cible est un noeud pour la rechercheService
    ...
    ifelse (has_statebase = 1)[
        ...
        ;show word "J'ai trouv? un service, je suis un " type_individual
        
        ;; L'individu est un suiveur
        ;show "Individu est dans le service ..."
        ;Deux m?thodes au cas ou on voudrai diff?rencier la gestion des chemins par individu
        ifelse (type_individual = 2 )[
        ...
        ;; FIXME : Possibilit? de transformer ce passage en methode
        ...
        ;;On passe dans l'?tat attente, on ne connai pas le stock du service a lavance
      ...
      ifelse has_statebase = 2 [
        ...
        ;Renvoie true si on est pret a consommer, cad si le ticket correspond et que l'on va etre servi
         ...
         if (t_pretAConsommer = true) [
           ...
           ask service waitingInWhatService [
            ...
            ;;FIXME : On peut tres bien imaginer un calcul de satisfaction qui soit impact? par l'attente
            ifelse besoin = 0 [
              ...
              ;;FIXME : faire un comportement reinit ? car comportement partag? avec closeService ici..
              ...
              ;; et donc inutile d'?craser le type_individual en le mettant a 1
        ...
        [
          ...
          [
             ...
             ;;Si waitingInWhatService != 0 alors c'est que l'on a quitt? la file d'attente sans etre servi !
             ;;On d?cremente
            ...
            ;;L'individu peut avoir mut? de suiveur a explorateur 
            ;; >> on fait donc un test sur le boolean de mutation
            ;;FIXME : on aurai pu interroger la m?moire si elle a un evtKnow, mais pour le moment on utilise un boolean !
            ...
            ;;On vide la m?moire de l'individu sur le tableau noir du quartier
  ...
  ;;FIXME : probleme avec les gens qui quittent les files d'attente anormalement ... on est oblig? de refaire un test qui ne rentre pas en conflit avec le reste ...
...
to-report obs_testLifeIndividual
  ...
  if (compteurTemps > 5 * Temps_Parents); 5 ticks = 1 minute (temps r?el / temps simul?)
    [
      report true
    ]
...
to CptService_incrFileAttente
  ...
  ; La file d'attente est incr?ment? de 1
  ;show "file d'attente + 1"
  set tailleFileAttente (tailleFileAttente + 1)
  ; Le nb est incr?ment? de 1
...
ifelse sum [sum achalandage] of services > 0 [
report 100 * count services * sum [sum achalandage] of services with [isMyService? = true] /  sum [sum achalandage] of services
][
report 0]
end
...
to CptIndividu_GoHomeSuiveur [pIndividu]
  ...
  let t_service_exist false
    ...
    ;On interroge sa m?moire pour savoir si on a trouv? le service correspondant au chemin initial ... utile car l'individu peut avoir chang? d'?tat en cours de chemin
    ...
    [
      ...
      [
        ...
        ;Si on trouve un service qui correspond a la liste m?moire ...
    ...
    ;show "Le service n'existe pas dans la m?moire"
    ;;On initialise la pile de connaissance avec un premier ?l?ment : le chemin ? suivre...
    CptIndividu_newKnownConnaissance "NobodyTargetChemin" t_cheminInitial cheminFutur nobody
    ]
  
end

; Quand l'individu ?met une satisfaction positive ou n?gative vis ? vis d'un chemin
...
to CptQuartier_knowNoChemin [pchemin]
  ...
  ;;Si le chemin existe encore, on d?cremente
  ;show pchemin
...
to CptQuartier_showCheminExplorateur [pquartier pchemin  ptemps pdist pservice]
  ...
  [
    ;On incr?mente la fr?quence du chemin
    ask t_objChemin [ cptChemin_incrFreq self ]
    
    ;Si la distance ou le temps est diff?rente du chemin initial alors on fait une mise ? jour
  ...
  ][; Le chemin n'est pas connu du quartier, on le cr??
  
    ;;Cr?ation du chemin, initialisation des parametres, frequence initialis? ? 1
...
to CptQuartier_showCheminSuiveur [pIndividu pquartier pchemin ptemps pdist pservice]
  ...
  ;;Remarque : Comme l'individu a pu passer de l'etat suiveur ? l'etat explorateur, on peut ne plus acc?der au chemin initialement pr?vu; on interroge donc la m?moire de l'individu
  let t_individu pIndividu 
  let t_cheminInitial item 0 (CptIndividu_AskMemories t_individu "evtKnow" "get:WhoCheminInit")
    
  ;; Passage des variables ? des variables temporaires
  let t_chemin pchemin
  let t_temps ptemps
  let t_dist pdist
  let t_service pservice
  ...
  ;;On teste si jamais le chemin a ?t? supprim? alors que quelqu'un d'autre l'utilise
  ;ifelse (chemin idCheminEnCours != nobody)
  ;[
    ...
    ;; FIXME on pourrait peut etre faire en sorte que le chemin ne soit pas test? si jamais la liste noeud correspond au chemin id suiveur ! 
    ;; On gagnerai en rapidit?.
    set t_objChemin (CptQuartier_searchConnaissanceExistant pquartier pchemin pservice)
 
  ;; Si il y'a un service + chemin qui existe 
  ifelse t_objChemin != nobody [
  
    ;Si ce n'est pas le service + chemin initialement pr?vu, alors on d?cr?mente l'ancien chemin et on incr?mente celui ci
    ...
    [
      ...
      if (chemin t_cheminInitial != nobody) 
      [
        ;;Remarque : La d?cr?mentation du chemin initial peut se discuter, il existe un tas de cas particulier qui gravite autour de cette hypoth?se ...
        ;; ex : Je rentre dans un service qui n'est pas le service attendu, je decremente, puis je ressort et tombe sur le service attendu, je reincr?mente ... un peu absurde non?
    ...
    [;Si le chemin correspond au chemin initialis?, on incr?mente la fr?quence de ce chemin
     ; Remarque : on peut ?tre pass? par plusieurs services, ce qui fait que finalement le chemin+service peut ?tre atteint ..
  ...
  ][  ;; Si il n'y aucune correspondance entre le chemin + service parcouru et le chemin et service trouv?
  
      ;;On d?cr?mente le chemin initial
      ;Le chemin peut avoir disparu entre temps
      if (chemin t_cheminInitial != nobody) 
      [
        ask chemin t_cheminInitial [ cptChemin_decrFreq self]
      ...
      ;;Cr?ation d'un nouveau chemin, initialisation des parametres
      let t_idChemin cptQuartier_createChemin t_idQuartier t_chemin 1 t_temps t_dist t_service
...
to cptChemin_decrFreq [chemin]
      ...
      [
        ...
        ;show word "Je d?cremente le chemin > " chemin
      ][
        ;show word "Je suis oubli? > " chemin
        die
      ]
      ...
      [
        ...
        let service item 6 (?)
            ...
            [
              ...
              ;FIXME : le mieux serait de cr?er le chemin au dernier moment ... avec les bon parametres...
              ;On ajoute le chemin au quartier, car pour le moment il est indif?renci?
  ...
  [
    ...
    [
      ...
      [
            ...
            [
                ;; Si le thickness est inf?rieur au thickness nouveau
...
to MajCheminQuartier [idQuartiers idIndividual]
    ...
    ;le bonheur par quartier est d?fini ci-dessous comme le bonheur moyen des individus qui y sont n?s, depuis le d?but
    ...
    ; le bonheur par quartier est d?fini ci-dessous comme le bonheur du dernier individu qui update le quartier
    ...
    ;set temp backgroundColor - 7 * bonheur / max [bonheur] of quartiers ;bonheur d?fini de facon relative aux autres
...
;    ][
;    set temp backgroundColor - 60 * bonheur ; bonheur d?fini de facon absolue
;
;    ]
    ]
    [set temp temp2]
...
to createServices [Nb point isMine?]
  ...
  while [not file-at-end?]
    ;; lecture des lignes du fichier et cr?ation du noeud
...
to createIndividuals [nb]  ;; cr?ation des individuals
...
to initIndividualSuiveur
      ...
      ;; Je recupere un chemin dans le quartier origine.Les chemins sont deja trie
        ...
        [
          ...
          ;; Soit p le chemin avec frequence la plus ?lev?, e le chemin avec le temps le plus court (efficace)
         ...
         set n (np / ne)
          ...
          ifelse (d >= gl_proportion_freq_dist * n)
            [
              ...
              ;show word "je prend le chemin le plus fr?quent? " [cheminFutur] of myself
               ;show word " val d = " d 
               ;show word " val n = " (gl_proportion_freq_dist * n )
            ]
...
ask quartiers [
   ...
   ;;ICI qu'on impl?mente le best distance, etc.
]
end

to initIndividualNoeudExplorateur
;; le code suivant permet de r?aliser l'accroche r?seau en projection sur le r?seau
;; pour diverses raisons, ce code n'est pas exploit? pour l'instant
...
to CptIndividu_init 
    ...
    ;;Le fait de reprendre des individu existant necessite un nettoyage correct de toute les variables ? zero, donc aucun r?sidu
    set noeudActuel 0
    set cible 0
    set has_muted false
    set estMort false
    ...
    set has_find_node false ; l'individu n'est pas sur un noeud par d?faut
    set has_find_service false ; l'individu n'a pas trouv? de service par d?faut
...
to CptIndividu_setOrientationReseauAtNode [idIndividual]
    ...
    ;La vitesse pour rejoindre le noeud est fix? arbitrairement ? 1
    set vitesseLinkActuel 1
...
to CptIndividu_setOrientation [idIndividual]
 ...
 ;show [compteurTemps] of idIndividual
  ...
  ;;La cible du tour pr?c?dent (qui existe forcement car has_terminate_true = true) devient le noeud actuel
  ; sauf dans le cas ou on se raccroche au reseau

    set Noeudactuel cible
  
  ; On reinitialise le marqueur dorientation a faux, jusqua la prochaine fois qu'on atteint le noeud tir?
  set [has_find_node] of idIndividual false
      ...
      [
        ;; Initialisation des noeuds sans avoir un chemin d?fini
      ...
      [
               ifelse (length [cheminFutur] of idIndividual > 0)[
                 ...
                 ;; Si il y'a un chemin futur qui est diff?rent de zero, c'est que l'on est un ancien suiveur, 
                 ;; on continue a avancer selon le chemin pr?d?fini tant que length > 0
    ...
    ;; On recupere la vitesse du link qui arrive,ca fait partie des comportements pr?alable ? l'?tat avancer  
    ;; On aurait pu d?cider ?galement de renseigner la vitesse quand has_node = true
    
      ;; on d?finit la vitesse sur le link a venir
...
to CptIndividu_changeToExplorateur
 ...
 to tirage_direction_alea [idIndividual]; nouvelle orientation al?atoire parmis les noeuds adjacents (liaison par un link)
...
; permet de faire avancer l'individual dans la direction de l'agent indiqu? en param?tre
; et de tester la pr?sence de services sur les actualLinks dont on est sur qu'ils en poss?dent
    ...
    ifelse (distance-nowrap direction >= vitesse )
      ...
      [
       ;; FIXME : A VERIFIER
       fd distance-nowrap direction  ;; si l'objectif est plus proche que le pas : on avance jusqu'? l'objectif
...
;; Consulter la liste des services m?moris? sur le chemin
;; FIXME : passer la m?moire en agent; on pourra meme memoriser les relations interIndividu ...
...
to-report CptIndividu_watchService [idIndividual]
  ...
  ;; les diff?renci? complique les choses et n'apporte au final pas grand chose, si au pire on veut savoir si le service est sur un noeud, on testera avec node-here.
  ;; FIXME : nous ne somme plus cens? revisiter un service dont on vient de sortir, donc watchService doit consulter la m?moire des services pour eviter de rerentrer dans le service.
   
   if (any? services in-radius vision)
   [
     let t_listeService 0
     let t_listeServiceRadius services in-radius vision
     ;;pour le moment on consid?re que le choix se fait de visu au hasard entre les services dans le champ de vision
     ;;FIXME : Am?lioration pour prendre le plus proche !!
     let oneOfServiceInRadius one-of t_listeServiceRadius
     
     ;;Est ce que le service que l'on voit est ferm??
     ...
     [
       ...
       ;On recupere la liste de service en m?moire 
       ...
       [
         ...
         [
           ;show word "Le service est en m?moire ... " ?
           report false
         ]
       ]
        ;show  "Le service n'est pas en m?moire, on va le stocker ... " 
        report true 
     ][
       report false
      ]
...
to CptIndividu_savePathMemories [idIndividual noeudToSave nodeExist ServiceExist]
  ...
  [
    ...
    [
      ; On doit tester ici que le noeud n'existe pas deja a n -1, effectivement, quand on sort d'un service, on repasse sur le noeud enregistr? ? l'entr?e du service ...
  ...
  ; Si le noeud existe, que le service aussi, mais que le service n'est pas sur le noeud en r?alit?
  ; Si le noeud existe , on pr?supose que l'on est sur le noeud pour faire le test ...
  ;---------------------------------------------------------------------------------------------------
  ;---------------------------------------------------------------------------------------------------
  ;---------------------------------------------------------------------------------------------------
  ;---------------------------------------------------------------------------------------------------
...
;     
;     
; dans ce cas on stocke le premier lien pr?vu, m?me si celui ci n'est pas effectu?
    ...
    [
      ; si on tombe sur un service alors qu'un noeud a deja ?t? parcouru et que l'on est pas sur un noeud, et que ce noeud n'existe pas deja dans la liste cheminParcouru
...
to CptIndividu_loadPathMemories [nodeExist]
  ...
  ;;Si on est un suiveur, on r?duit la m?moire du cheminFutur de 1 a chaque fois que l'on passe un nouveau noeud
  ...
  [
    ;;C'est un tableau diff?rent de zero
    if (length CheminFutur > 0)
    [
      ;; On r?duit le tableau de 1
...
to-report CptService_askTraitementFileAttente [numeroAttente]
  ...
  ;La file d'attente r?pond au demande tout les n ticks, chose que l'on peut r?gler via un curseur
...
to-report CptService_repondBesoin [ individu ]
  ...
  ifelse (stock > 0)
  [    
       ;;FIXME : Rajouter la gestion des paquet d?livr? par le service
...
to CptService_closeService
    ...
    without-interruption
    [
      ;;Permet de verifier que l'on est bien dans un ?tat ferm? ...       
...
to gestionService  ;; gestion des stocks des services et de l'apetit des individuals.
  ...
  ;On tire le prochain individu a etre appel?, seulement dans la file d'attente
...
;      if lastSales != []
;      [
;      ;set stock floor (stock + parRenewal * (mean lastSales))] ;formule g?rant le fonctionnement des boulangeries
;      set stock stock + floor (parRenewal * (mean lastSales))] ;formule g?rant le fonctionnement des boulangeries
;      ;choix ? faire sur la formuler
...
;
;set-current-plot "Distance (m?tres)"
;  set-plot-x-range 0 1
;  set-plot-y-range 0 1
...
;let tempI mean [best_temps] of quartiers with [nb_individu_tot > 0]
;ifelse tempI > 0 [
;set graphe_temps2 (standard-deviation [best_temps] of quartiers with [nb_individu_tot > 0] / mean [best_temps] of quartiers with [nb_individu_tot > 0])
;][
;set graphe_temps2 0
;]

set graphe_temps2 standard-deviation [best_temps] of quartiers with [nb_individu_tot > 0]
 if graphe_temps2 > max_graphe_temps2 [set max_graphe_temps2 round (graphe_temps2) + 1]
set-current-plot "Inegalite Temps"
;  ;set-plot-x-range 0 1
;  ;set-plot-y-range 0 1
...
;; IMPORT ZONES :Lecture dans le fichier texte des valeurs ? 
;; implementer pour l'attribut "idQuartier" de chaque patches
to import-iris
...
;; Attribution d'une couleur aux patches ayant une valeur diff?rente de -9999
to setupTemp
  cd
  ct
...
;--------------------
;Ex?cution de l'importation du RESEAU
; 1?) import-noeud
; 2?) import-lien
;--------------------
...
;; Cette proc?dure lit dans le fichier texte les NOEUDS du graphe
to import-noeud
  ;; Ouverture du fichier
  ...
  [
    ...
    let items read-from-string (word "[" file-read-line "]")
    create-nodes 1 [
    ;; pour chaque ligne, on extrait 3 informations : identifiant du noeud, coordonn?e x et coordonn?e y
...
;; Cette proc?dure lit dans le fichier texte les LIENS du graphe
to import-lien
  ;; Ouverture du fichier
  ...
  while [not file-at-end?]
  [
    ;; lecture du service de d?part et du service d'arriv?e dans le fichier texte
    ;; Cr?ation du lien entre les services
    let items read-from-string (word "[" file-read-line "]")
...
;; Renvoie l'identifiant du noeud recherch?
...
to createQuartiers
  ...
  [
    create-quartiers 1 [
      set id ?1
      set txPop ?2
      ; lignes ? utiliser pour "mat?rialiser" les quartiers (utile pour le debugging)
...
fichier_lien
fichier_lien
"paris12_rues_L.txt" "paris12_rues_L_u.txt" "poly_rues_L_mono.txt" "paris12_rues_L_u2.txt" "poly_rues_L_poly.txt" "manhattan_rues_L.txt" "bastide_rues_L.txt" "carre_rues_L.txt"
3
...
447
Au moins : 
Nbindividuals * Somme_par_Habitant
17
1
11
...
Accessim est une simulation ? base d?agent qui est cens? repr?senter de mani?re p?dagogique et interactive les d?placements et accessibilit?s dans une ville. Cette simulation est programm?e sur la plateforme gratuite NetLogo qui permet de r?aliser des simulations sans ?tre n?cessairement un d?veloppeur confirm?.
Cette simulation est compos?e de plusieurs agents qui sont : des individus, des quartiers, des services et le r?seau. 
Les agents individus sont r?gis par des r?gles de comportements qui leur permettent de se d?placer sur le r?seau, de rentrer et de consommer dans les services. 
Les agents services sont plac?s sur le r?seau et offre des services aux individus. Ces services g?rent un stock et peuvent fermer lorsqu?il arrive ? ?ch?ance.
Pour le ? d?cideur en herbe ? il est possible de visualiser un certain nombre d?informations qui va influencer le placement des services : l?accessibilit? par quartier, le taux de satisfaction des individus, la distance moyenne pour atteindre un service, les ventes totales, etc.
L?objectif du mod?le ?tant d??tre interactif l?utilisateur peut rajouter ou enlever des services afin de voir de mani?re dynamique les accessibilit?s qui ?voluent dans les quartiers et dans la ville. Un certain nombre de param?tres peut ?galement ?tre modifi?  via le tableau de bord de la mod?lisation : choix de la densit? de population, quantit? de services ? implanter, vitesse des individus, etc.
...
Les individus se d?placent exclusivement sur ce r?seau jusqu?? ce qu?ils trouvent un service, ou que leur dur?e de parcours soit ?coul?e.
Les services peuvent ?tre d?pos?s sur les n?uds du r?seau par les utilisateurs.
Ce r?seau est constitu? de n?uds fixes reli?s par des arcs non valu?s. NetLogo permet l?utilisation d?agent ? link ? pour mod?liser les liens entre plusieurs agents n?uds. La localisation et les relations entre n?uds sont charg?es depuis un fichier texte.

2) Les individus 

a) Apparition dans le mod?le
Il existe deux types d?individus dans le mod?le : suiveur et explorateur. Ils sont impl?ment?s sous la forme d?un seul agent ayant une variable sp?cifiant le type d?individu.  
Les individus apparaissent dans le mod?le en fonction des densit?s de population du quartier, on utilise une m?thode de criblage : un quartier ayant une forte densit? de population aura plus de chance ? chaque tour de faire appara?tre un individu dans la simulation qu?un quartier ayant une plus faible densit? de population. 
b) Le cas de l?individu explorateur 
L?individu explorateur se d?place de mani?re al?atoire sur le du r?seau. 
Lorsque l?individu passe sur un n?ud, ce dernier est stock? dans une liste interne de l?agent individu. Lors de la rencontre d?un explorateur avec un service, un calcul de satisfaction est r?alis? qui prend en compte le temps de parcours (inverse du temps mis par l?individu avant d?acc?der au service) et le pourcentage d?app?tit satisfait par le service. Cet indice de satisfaction est enregistr? en ent?te de la liste de n?ud ayant permis d?acc?der au service. Quand l?individu explorateur rentre dans son quartier, il transmet cette liste ? son quartier d?origine et dispara?t. Cette liste sera ce qu?on appelle un ? chemin ? par la suite. Un autre individu r?apparait imm?diatement ailleurs dans cette simulation.
Un individu explorateur qui n?a pas trouv? de service sur son chemin ne transmet pas la liste de n?uds parcourus ? son quartier.
c) Le cas de l?individu suiveur :
Lorsqu?il appara?t, cet individu lance un tirage probabiliste pour d?terminer s?il va demander ? trier les chemins existants en fonction de la satisfaction. Il r?cup?re dans tous les cas le premier ?l?ment de la liste, qui n?est pas forc?ment le meilleur chemin, pour initialiser son futur chemin parcouru.  
Si le suiveur rencontre un autre service que celui pour lequel il est destin?, il enregistre l?information de la m?me mani?re que le ferait un individu explorateur : il rentre dans le service, calcule l?indice de satisfaction et rentre chez lui. Le quartier sera mis ? jour avec l?ajout de ce nouveau chemin. Il n?y a aucun test dans ce mod?le qui permette ? l?individu de diff?rencier un service qui a ?t? d?j? r?pertori? d?un autre service, ce qui engendre assez vite la cr?ation d?un certain nombre de chemins doublons dans la liste de chemins du quartier.
d) Sch?ma bilan de la logique individu :
Attention, les fonctions ne sont pas d?taill?es dans ce sch?ma et comportent des logiques autres que celles qui sont sous entendues dans le nom des fonctions. Il est donc impossible de comparer ce mod?le avec le diagramme d?activit? qui est pr?sent? pour la nouvelle architecture de l?agent individu.

 
  


L?indice de satisfaction des individus est calcul? en divisant le besoin par la part d?livr?e par le service. On multiplie ensuite le r?sultat de ce calcul par l?inverse de la distance parcourue. 
Par exemple, si le besoin de l?individu est de trois unit?s et que le service n?a plus que deux unit?s, alors le taux de satisfaction sera de deux tiers multipli? par l?inverse de la distance. Dans ce cas l? l?individu rentrera chez lui pas totalement satisfait. 
Dans le cas o? le service n?a plus de stock et ne peut rien lui d?livrer alors il rentrera chez lui insatisfait. 
Dans l??volution du mod?le nous verrons comment l?individu sera autoris? ? chercher un autre service lorsqu?il est insatisfait.
Le retour de chaque individu se fait en fonction d?une horloge interne (fix?e de mani?re exog?ne par une variable sur le tableau de bord de la simulation) qui indique ? partir de quel moment celui ci retourne ? son domicile, qu?il ait trouv? un service ou non.

3) Agent Quartier

a) Apparition dans le mod?le
Chaque quartier est un agent, incapable de bouger comme les individus, invisible aux yeux de l?utilisateur. C?est  pourtant lui qui est le c?ur du mod?le. Effectivement ils sont charg?s de calculer le taux de ? bonheur ? des quartiers ? partir des connaissances rapport?es par les individus. C?est cette information que l?on affiche ensuite aux d?cideurs  charg?s d?impl?menter les futurs services. On utilise un cama?eu de rose pour repr?senter ce taux de ? bonheur ?.
b) Enregistrement des connaissances
Inspir? de l?architecture informatique nomm? ? blackboard  ? (tableau noir) chaque individu qui a rencontr? un service ajoute son information ? un tableau noir g?r? par le quartier (impl?ment? par une liste).
Dans notre mod?le la probl?matique sous jacente au tableau noir quartier est de trouver le chemin vers un service qui a le plus grand taux de satisfaction possible. Les individus, lorsqu?ils rencontrent un nouveau service, apportent une r?ponse partielle ? cette probl?matique. D?s lors,  pass? un certain nombre d?it?ration, ce tableau noir va converger vers une solution qui pr?sentera effectivement le ? meilleur chemin ?. 
A chaque d?part d?un individu suiveur le tableau noir est tri? et l?individu prend le chemin ayant le meilleur indice de satisfaction.
Nous verrons dans la suite du rapport comment ?tendre ce mod?le pour g?rer ? la fois des cas plus complexes d?enregistrement de chemin et  la possibilit? pour les individus de choisir  le chemin autrement qu?en prenant le meilleur en satisfaction.
c). Affichage du taux de bonheur
Cet indice bonheur est calcul? en r?alisant une moyenne ?volutive des taux de satisfaction ramen?s par les individus. On d?cide dans cette moyenne de donner un poids plus important au pass? r?cent, le poids des taux de satisfaction des individus r?cemment arriv?s ?tant plus ?lev? que les poids des taux de satisfaction pass?s. C?est une mani?re de rendre l??volution du taux de bonheur plus dynamique.

4) Les agents services

 Gestion des stocks , bas?e sur une approximation ici car cette ?quation est d?crite avec l?utilisation d?une loi normale, ou une loi poisson, calcul?e sur la base d?une fr?quentation du service pour l?ensemble des jours pr?c?dents. Notre mod?le impl?mente seulement une ?quation qui se base sur les derni?res ventes.
Cette nouvelle gestion des stocks se fait sur la base de plusieurs param?tres fix?s de mani?re exog?ne via le tableau de bord de la simulation :
?	La p?riode de recalcul des stocks
?	Le nombre de client insatisfait
?	Le nombre de client satisfait
?	Le prix d?achat d?une unit? de stock
?	Le prix de vente d?une unit? ? un individu
?	La perte du ? un invendu
?	La perte du ? un client insatisfait
?	Le loyer ? payer ? chaque pas de temps
Tous les param?tres n?influent pas sur le calcul du nouveau stock, seul le nombre de clients satisfaits, la perte due ? un invendu, la perte due ? un client insatisfait rentre en compte dans l??quation.
La fermeture du service intervient lorsque celui-ci n?a plus la capacit? ? acheter des stocks ou qu?il n?est plus en mesure de payer le loyer.
...
L'utilisateur commence par choisir la forme du r?seau viaire de la ville : il peut choisir parmi plusieurs r?seaux pr?d?finis : r?seau radioconcentrique, en grille r?guli?re, ou bien choisir le r?seau de rues du 12?me arrondissement parisien, etc.. (param?tre ...). Il choisit ensuite la forme de la r?partition de l'habitat sur ce r?seau viaire. L? encore, on peut choisir parmi plusieurs configurations pr?-?tablies : 

THINGS TO NOTICE
----------------
I. Hypoth?ses du mod?le 
Dans le mod?le Accessim initial, le chemin s?lectionn? par un individu ? l?initialisation est celui qui poss?de le meilleur indice de satisfaction.
Cet indice est calcul? en tenant compte de l?inverse de la distance au service et du pourcentage de satisfaction de l?individu.
Rapidement s?est pos?e la question d?un autre moyen pour g?rer le choix du chemin. Nous somme parti de l?hypoth?se qu?un individu ne choisi pas seulement d?acc?der ? un service parce qu?il est le plus pr?s, mais aussi parce que ce service est plus ou moins fr?quent?.
I. Description de l?algorithme impl?ment? 
Lors de l?initialisation d?un individu, le quartier demande ? r?cup?rer dans sa base de chemin connue les identifiants de deux chemins : le premier doit ?tre celui qui poss?de la fr?quence la plus ?lev?e de parcours, le deuxi?me doit ?tre celui qui a le temps de parcours le plus court.
L?algorithme a ?t? propos? par un de mes tuteurs de stage, je n?ai fait que l?impl?menter.  
1. Initialisation des variables 
On consid?re un chemin p qui est le plus probable, et un chemin e qui est le plus efficace.
Le chemin p est le maximum fr?quence,
Le chemin e le plus efficace est celui qui poss?de le temps minimum de parcours,
Alpha est un param?tre exog?ne pour modifier le poids de la fr?quence dans le calcul du rapport final
1. Calcul des rapports 
Soit n le r?sultat du rapport entre la fr?quence du chemin p et la fr?quence du chemin e
Soit d le r?sultat du rapport entre la distance du chemin p et la distance du chemin e
Si d > alpha * n alors on prend le chemin le plus court, c'est-?-dire le chemin p
Si d < alpha * n alors on prend le chemin le plus fr?quent?, c'est-?-dire le chemin e
2. Exemple concret 
Le chemin n ? une fr?quence de 20, et un temps de parcours de 20
Le chemin p ? une fr?quence de 1 et un temps de parcours de 19
Si alpha vaut 1 alors n = 10 et d = 1, 05, d est donc inf?rieur ? n, c?est le chemin le plus fr?quent? qui est choisi par l?individu.
La diff?rence de temps de parcours ne suffit pas ? effacer la diff?rence de fr?quentation. Autrement dit, pour contrebalancer la diff?rence de fr?quence dans le choix du chemin, il faudrait que la diff?rence de temps de parcours soit beaucoup plus importante. 
En modifiant le param?tre alpha, le mod?lisateur d?cide de donner un poids plus ou moins important (entre 0 et 1 sur le tableau de bord) au rapport de fr?quence des chemins dans le r?sultat final.
Cette ?bauche d?algorithme de comportement est une tentative pour simuler le fait que les individus ont tendance ? suivre un m?me chemin si celui-ci est tr?s fr?quent?, et cela m?me si un autre chemin, beaucoup plus court, existe. 
...
QU'est ce que l'?quit? en termes d'accessibilit? ? Quelles localisations des ressources apparaissent optimales ou au contraire conduisent ? l'exclusion des territoires ?

L'objectif est de montrer que la r?partition des services ou ?quipements est le plus souvent in?gale, et que leur acc?s est diff?rent selon les territoires, qui peuvent ?tre bien desservis ou exclus, cr?ant ainsi des in?galit?s de satisfaction entre les habitants. 
L'objectif de ce mod?le est d'illustrer ces id?es dans un cadre tr?s simplifi? : l'acc?s dans un contexte urbain ? un commerce de proximit? : des boulangeries. Les individus ? la recherche de ce service de proximit? sont ici des enfants. 
...
Le mod?le simule des habitants en ville, ici des enfants. Les ?quipements sont des boulangeries. Il y a donc deux types d'agents interragissant dans l'environnement urbain : des agents enfants, et des agents boulangeries. Le comportement de chaque type d'agent est r?gi et contraint par quelques r?gles tr?s simples : 

- les enfants parcourent les rues pour trouver une boulangerie et consommer des pains au chocolat. Ils ont pour cela un temps limit? (param?tre ...)

- les boulangeries doivent adapter leurs stocks ? la demande qui d?pend de la client?le potentielle et du contexte contexte concurrentiel. L'ajustement de leur stock est r?gi par les param?tres ... et ... . Si il s'?coule plus de ... it?rations sans qu'elle ait renouvell? son stock, une boulangerie ferme, et l'agent est supprim?. 
